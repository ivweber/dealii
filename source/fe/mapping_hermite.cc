...
template <int dim, int spacedim>
std::size_t
MappingHermite<dim, spacedim>::InternalData::memory_consumption() const
{
...
template <int dim, int spacedim>
bool
MappingHermite<dim, spacedim>::is_compatible_with(
...
template <int dim, int spacedim>
UpdateFlags
MappingHermite<dim, spacedim>::requires_update_flags(
...
MappingHermite<dim, spacedim>::get_data(const UpdateFlags      update_flags,
...
template <int dim, int spacedim>
std::unique_ptr<typename Mapping<dim, spacedim>::InternalDataBase>
MappingHermite<dim, spacedim>::get_face_data(
...
template <int dim, int spacedim>
std::unique_ptr<typename Mapping<dim, spacedim>::InternalDataBase>
MappingHermite<dim, spacedim>::get_subface_data(
  const UpdateFlags          update_flags,
...
template <int dim, int spacedim>
void
MappingHermite<dim, spacedim>::update_cell_extents(
...
template <int dim, int spacedim>
void
MappingHermite<dim, spacedim>::maybe_update_cell_quadrature_points(
  const typename Triangulation<dim, spacedim>::cell_iterator &cell,
...
template <int dim, int spacedim>
void
MappingHermite<dim, spacedim>::maybe_update_face_quadrature_points(
  const typename Triangulation<dim, spacedim>::cell_iterator &cell,
...
template <int dim, int spacedim>
void
MappingHermite<dim, spacedim>::maybe_update_subface_quadrature_points(
  const typename Triangulation<dim, spacedim>::cell_iterator &cell,
  const unsigned int                                          face_no,
...
template <int dim, int spacedim>
void
MappingHermite<dim, spacedim>::transform_quadrature_points(
  const typename Triangulation<dim, spacedim>::cell_iterator &cell,
  const InternalData &                                        data,
...
template <int dim, int spacedim>
void
MappingHermite<dim, spacedim>::maybe_update_normal_vectors(
  const unsigned int           face_no,
  const InternalData &         data,
...
template <int dim, int spacedim>
void
MappingHermite<dim, spacedim>::maybe_update_jacobian_derivatives(
  const InternalData &             data,
  const CellSimilarity::Similarity cell_similarity,
...
MappingHermite<dim, spacedim>::fill_fe_values(
  const typename Triangulation<dim, spacedim>::cell_iterator &cell,
  const CellSimilarity::Similarity                            cell_similarity,
    ...
    &output_data) const
{
  ...
  if (data.update_each & update_values)
    if (cell_similarity != CellSimilarity::translation)
      {
        const unsigned int dofs = output_data.fe_values[0].size();
        ...
        switch (dim)
          {
            case 1:
                for (unsigned int i = 0; i < output_data.fe_values.size(); ++i)
                {
                    factor_1 = 1.0;
                    for (unsigned int d1 = 0; d1 <= regularity; ++d1)
                    {
                        output_data.fe_values[i][d1] *= factor_1;
                        output_data.fe_values[i][d1 + regularity + 1] *= factor_1;
                        factor_1 *= data.cell_extents[0];
                    }
                }
                break;
            case 2:
                for (unsigned int i = 0; i < output_data.fe_values.size(); ++i)
                {
                    ...
                    for (unsigned int d2 = 0; d2 <= regularity; ++d2)
                    {
                        for (unsigned int d1 = 0; d1 <= regularity; ++d1)
                        {
                            output_data.fe_values[i][d1 + d2 * dofs_per_dim]                                         *= factor_1 * factor_2;
                            output_data.fe_values[i][d1 + regularity + 1 + d2 * dofs_per_dim]                        *= factor_1 * factor_2;
                            output_data.fe_values[i][d1 + (d2 + regularity + 1) * dofs_per_dim]                      *= factor_1 * factor_2;
                            output_data.fe_values[i][d1 + d2 * dofs_per_dim + (regularity + 1) * (dofs_per_dim + 1)] *= factor_1 * factor_2;
                            factor_1 *= data.cell_extents[0];
                        }
                        factor_2 *= data.cell_extents[1];
                    }
                }
                break;
            case 3:
                for (unsigned int i = 0; i < output_data.fe_values.size(); ++i)
                {
                    ...
                    for (unsigned int d3 = 0; d3 <= regularity; ++d3)
                    {
                        for (unsigned int d2 = 0; d2 <= regularity; ++d2)
                        {
                            for (unsigned int d1 = 0; d1 <= regularity; ++d1)
                            {
                                output_data.fe_values[i][d1 
                                        + d2 * dofs_per_dim 
                                        + d3 * dofs_per_dim * dofs_per_dim] *= 
                                    factor_1 * factor_2 * factor_3;
                                output_data.fe_values[i][d1 + regularity + 1 
                                        + d2 * dofs_per_dim 
                                        + d3 * dofs_per_dim * dofs_per_dim] *= 
                                    factor_1 * factor_2 * factor_3;
                                output_data.fe_values[i][d1 
...
template <int dim, int spacedim>
void
MappingHermite<dim, spacedim>::fill_fe_face_values(
  const typename Triangulation<dim, spacedim>::cell_iterator &cell,
  const unsigned int                                          face_no,
...
template <int dim, int spacedim>
void
MappingHermite<dim, spacedim>::fill_fe_subface_values(
  const typename Triangulation<dim, spacedim>::cell_iterator &cell,
  const unsigned int                                          face_no,
...
template <int dim, int spacedim>
void
MappingHermite<dim, spacedim>::transform(
...
template <int dim, int spacedim>
Point<spacedim>
MappingHermite<dim, spacedim>::transform_unit_to_real_cell(
  const typename Triangulation<dim, spacedim>::cell_iterator &cell,
...
template <int dim, int spacedim>
Point<dim>
MappingHermite<dim, spacedim>::transform_real_to_unit_cell(
  const typename Triangulation<dim, spacedim>::cell_iterator &cell,
...
{
  return std::make_unique<MappingHermite<dim, spacedim>>(*this);
}

// instantiations
#include "mapping_hermite.inst.in"
