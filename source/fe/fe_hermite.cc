...
namespace internal
{
                ...
                for (unsigned int dk = 0; dk < 2; ++dk)
                    for (unsigned int dl = 0; dl < 2; ++dl)
                    {
                        for (unsigned int i = 0; i < node_dofs_1d; ++i)
                            for (unsigned int j = 0; j < node_dofs_1d; ++j)
                                for (unsigned int k = 0; k < node_dofs_1d; ++k)
                                    for (unsigned int l = 0; l < node_dofs_1d; ++l)
                                        h2l[l + k * node_dofs_1d + j * node_dofs_2d +
                                            i * node_dofs_3d + offset] =
                                                l + k * dim_dofs_1d + j * dim_dofs_2d +
                                                i * dim_dofs_3d +
                                                node_dofs_1d * (dl + dk * dim_dofs_1d 
                                                    + dj * dim_dofs_2d + di * dim_dofs_3d);
  ...
  hermite_hierarchic_to_lexicographic_numbering(const unsigned int regularity)
  {
    const std::vector<unsigned int> dpo = get_hermite_dpo_vector(dim, regularity);
  ...
  {    
    return Utilities::invert_permutation(hermite_hierarchic_to_lexicographic_numbering<dim>(regularity));
  }
  ...
  template <int dim>
  inline std::vector<unsigned int>
  hermite_face_lexicographic_to_hierarchic_numbering(const unsigned int regularity)
  {
    return (dim > 1) ? hermite_lexicographic_to_hierarchic_numbering<dim - 1>(regularity) : std::vector<unsigned int>();
  }
  ...
  public:
      void
      rescale_fe_hermite_values(const FE_Hermite<xdim, xspacedim> &                           fe_herm,
                                ...
                                Table<2, Number> &                                        value_list)
      {
          ...
          AssertDimension(dofs, 2 * regularity + 2);
          
          const unsigned int q_points = value_list.size(1);
          AssertIndexRange(q_points, mapping_data.quadrature_points.size() + 1);
        
          std::vector<unsigned int> l2h = hermite_lexicographic_to_hierarchic_numbering<1>(regularity);
                                ...
                                Table<2, Number> &                                        value_list)
      {
          const unsigned int dofs = fe_herm.n_dofs_per_cell();
          const unsigned int regularity = fe_herm.get_regularity();
          const unsigned int dofs_per_dim = 2 * regularity + 2;
          const unsigned int q_points = value_list.size(1);
          (void)dofs;
          ...
          std::vector<unsigned int> l2h = hermite_lexicographic_to_hierarchic_numbering<2>(regularity);
                                ...
                                Table<2, Number> &                                        value_list)
      {
          ...
          std::vector<unsigned int> l2h = hermite_lexicographic_to_hierarchic_numbering<3>(regularity);
...
//TODO: re-write the following to use deal.II functionality for matrices
/*
...
struct FE_Hermite<xdim, xspacedim>::Implementation
{
  ...
  initialise_constraints(FE_Hermite<1, spacedim> &fe_hermite)
  {
    ...
    for (unsigned int i = 0; i < sz; ++i)
        fe_hermite.interface_constraints(i,i) = 1.0 / std::pow(2, i);
  ...
  //TODO: This function probably needs to be rewritten
  template <int spacedim>
  static void
  initialise_constraints(FE_Hermite<2, spacedim> &fe_hermite)   
  {
    ...
    std::vector<unsigned int> face_index_map =
        internal::hermite_face_lexicographic_to_hierarchic_numbering<2>(regularity);
  ...
  //TODO: Check if this function needs to be rewritten
  template <int spacedim>
  static void
  initialise_constraints(FE_Hermite<3, spacedim> &fe_hermite)   
  {
    ...
    std::vector<unsigned int> face_index_map =
        internal::hermite_face_lexicographic_to_hierarchic_numbering<3>(regularity);
    ...
    for (unsigned int i = 0; i < sz2; ++i)
        for (unsigned int j = 0; j < 4 * sz * sz; ++j)
        {
            ...
            fe_hermite.interface_constraints(i + 12 * sz2,
                                        face_index_map[j + 2 * sz2]) = entry_1;
            fe_hermite.interface_constraints(i + 15 * sz2,
                                        face_index_map[j + 3 * sz2]) = entry_1;
        }
        ...
        for (unsigned int j = 0; j < sz; ++j)
        {
            ...
            for (unsigned int k = 0; k < sz; ++k)
                for (unsigned int l = 0; l < sz; ++l)
                {
                    ...
                    fe_hermite.interface_constraints(local_1,
                                                face_index_map[local_2]) = entry_3;
            ...
            for (unsigned int k = 0; k < sz; ++k)
                for (unsigned int l = 0; l < sz; ++l)
                {
                    ...
                    fe_hermite.interface_constraints(local_1,
                                                 face_index_map[local_2]) = entry_1;
                    fe_hermite.interface_constraints(
...
};  //struct FE_Hermite::Implementation
...
FE_Hermite<dim, spacedim>::FE_Hermite(const unsigned int reg)
  : FE_Poly<dim, spacedim>(
      ...
      FiniteElementData<dim>(internal::get_hermite_dpo_vector(dim, reg),
                             1,
                             2 * reg + 1,
    ...
    &output_data) const
{
  ...
  const typename FE_Hermite<dim,spacedim>::InternalData &fe_data 
        = static_cast<const typename FE_Hermite<dim,spacedim>::InternalData &>(fe_internal);
...
namespace VectorTools
{
    ...
    namespace internal
    {
                                              ...
                                              types::global_dof_index &                                              no_constrained_dofs)
        {
            ...
            const unsigned int regularity = dynamic_cast<const FE_Hermite<dim>&>(dof_handler.get_fe()).get_regularity();
            ...
            const std::vector<unsigned int> l2h 
                = dealii::internal::hermite_lexicographic_to_hierarchic_numbering<dim>(regularity);
                            ...
                            std::vector<unsigned int>                                              component_mapping)
    {
        ...
        Assert((dynamic_cast<const FE_Hermite<dim>*>(&dof_handler.get_fe()) != nullptr),
                ExcMessage("The version of project boundary functions with a MappingHermite object as the mapping argument"
                           " should only be used with a DOF handler linked to an FE_Hermite object."));
