...
{
    FEValues<dim> herm_vals(mapping, dof.get_fe(), quadrature, update_values | update_quadrature_points | update_JxW_values);
    std::vector<types::global_dof_index> local_to_global(dof.get_fe().n_dofs_per_cell());
    double err_sq = 0;
    ...
    for (const auto &cell: dof.active_cell_iterators())
    {
        ...
        for (const unsigned int q : herm_vals.quadrature_point_indices())
        {
            ...
            for (const unsigned int i : herm_vals.dof_indices())
                diff -= fe_solution(local_to_global[i]) * herm_vals.shape_value(i,q);
...
test_wave_solver(const double initial_time, const unsigned int regularity)
{
    ...
    Solution_derivative<dim> wave_tdev(wave);
    ...
    //Make an initial time-step using u(t+dt) = u(t) + dt u_t(t) + 0.5 dt^2 M^(-1)A u(t) + O(dt^3, dt^2 dx^(p+1))
